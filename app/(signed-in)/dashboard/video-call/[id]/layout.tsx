...
import {Call, CallingState, StreamCall, StreamTheme, StreamVideo, StreamVideoClient} from '@stream-io/video-react-sdk'
...
if (error) {
    return (
        <StatusCard
            ...
            action={<button onClick={() => window.location.reload()} className='w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2'>Retry</button>}
        >
...
if (!call) {
    return (
        <StatusCard title='Joining call...' className='min-h-screen bg-green-50'>
            ...
            <div className="text-green-600 font-mono text-sm bg-green-100 px-3 py-1 rounded-full inline-block">Call ID: {id}</div>
        </StatusCard>
    )
}
